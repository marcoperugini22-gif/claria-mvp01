...
export default function SignupPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    // Se la sessione è già disponibile (email confirmation disabilitata)
    if (data.session) {
    ...
    // Altrimenti chiediamo di verificare l'email
    setDone(true);
    setLoading(false);
  }
    ...
    return (
      <main className="min-h-dvh px-6 pt-12 pb-8 flex flex-col max-w-md mx-auto">
        ...
        <div className="flex-1 flex flex-col justify-center text-center">
          ...
          <p className="mt-5 text-[12px] text-claria-ink/45 leading-relaxed">
    ...
    <main className="min-h-dvh px-6 pt-12 pb-8 flex flex-col max-w-md mx-auto">
      ...
      <div className="mt-8 text-center">
        <p className="text-[13px] text-claria-ink/60">
          Hai già un account?{" "}
          <Link href="/auth/login" className="font-medium text-claria-ink underline underline-offset-2">
            Accedi
          </Link>
        </p>
      </div>
